...
export default function Chat() {
  ...
  // after setting process.env.REACT_APP_NAME, need to restart server
  ...
  const filteredChats = chats.filter(c=>c.user.name.toLowerCase().includes(inputText.toLowerCase()))
  ...
  return (
    ...
    <Drawer open={open} onClose={() => toggleDrawer(false)} anchor="left">
        <div style={{ width: "370px" }}>
          <LoginUserDetails toggleDrawer={toggleDrawer} currentUserId={currentUserId} currentUserStatus={currentUserStatus}/>
        </div>
      </Drawer>
    </>
  );
